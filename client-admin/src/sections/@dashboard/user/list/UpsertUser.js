...
export default function UpsertUser() {
  ...
  const { isEdit, currentUser } = userState;
  const { roleList } = useRecoilValue(Role);
  const { user } = useRecoilValue(Authentication);
  const ROLE_OPTIONS = roleList
    .filter((role) => (user.role === 'root' ? (!isEdit ? role.name !== 'root' : role) : role.name !== 'root'))
    .map((role) => role.name);
  ...
  const defaultValues = useMemo(
    () => ({
      ...
      role: currentUser?.role || ([...roleList]?.sort((cur, next) => cur.priority - next.priority))[0].name,
