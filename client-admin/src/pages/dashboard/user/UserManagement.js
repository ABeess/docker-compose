...
import { getAllUser, singleDeleteUser } from 'src/api/user.api';
...
export default function UserList() {
  ...
  const { roleList } = roleState;
  const { userList, isEdit, total } = userState;
  const [filterEmail, setFilterEmail] = useState('');
  const { enqueueSnackbar } = useSnackbar();

  const ROLE_OPTIONS = [
    'all',
    ...roleList?.filter((role) => (user.role === 'root' ? role : role.name !== 'root')).map((role) => role?.name)
  ];
          ...
          <Scrollbar>
            <TableContainer sx={{ minWidth: 800, position: 'relative' }}>
              <Table size={dense ? 'small' : 'medium'}>
                ...
                <TableBody>
                  {userList
                    ?.filter((eachUser) => (user?.role === 'root' ? eachUser : eachUser.role !== 'root'))
                    ?.map((row) => (
                      <UserTableRow
                        key={row._id}
                        row={row}
